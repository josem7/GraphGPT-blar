...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        index = self.list_data_dict[i]["index"]
        if isinstance(i, int):
            sources = [sources]
        ...
        data_dict['graph_data'] = self.graph_data_all[int(index)]
        return data_dict
...
def train():
    ...
    res = parser.parse_args_into_dataclasses(return_remaining_strings=True)
    model_args, data_args, training_args = res[0:3]
